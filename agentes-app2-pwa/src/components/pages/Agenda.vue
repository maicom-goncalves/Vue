  ...
  <div class="newUsers">
    <form v-on:submit.prevent="addUser" v-if="!enviado" id="newUsers">
      <div class="entradas">
        ...
        <p><b>Agente</b></p>
        <p><b>Nome do agente :</b>{{agente.nome}}</p>
        <p>
          <b>CNS do agente : </b>{{agente.CNS}}
        </p>
        <div class="turno">
      ...
      <div class="informacoes">
        <p >
          <b>Nome:</b>{{pegarNome(morador.nome)}} {{ morador.sobrenome }}
          </p>
...
export default {
  ...
  firestore() {
    return {
      ...
      morador: db.collection("morador").doc(this.aldeias).collection("moradores").doc(this.id3),
      agente: db.collection("agente").doc(this.aldeias).collection("agentes").doc(this.id)
    };
  },
  data() {
    return {
  ...
  created() {
    console.log(this.nomeMorador+" vizualizar")
  },
  methods: {
    pegarNome:function(nome){
  ...
  mounted: function () {
    ...
    this.nomeMorador =this.pegarNome();
  },
};
</script>
  <style>
